...
impl<'info> InitializePrivacyConfig<'info> {
    pub fn initialize_privacy_config(
        &mut self,
        privacy_enabled: bool,
        min_allowlist_size: u32,
        bumps: &InitializePrivacyConfigBumps,
    ) -> Result<()> {
        require!(
        ...
        privacy_config.stablecoin = self.stablecoin.key();
        privacy_config.authority = self.authority.key();
        ...
        privacy_config.bump = bumps.privacy_config;
