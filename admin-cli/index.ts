...
program
  .command("set-hook-sss1")
  .requiredOption("-p, --pda <string>", "Stablecoin PDA")
  .requiredOption("-m, --mint <string>", "Mint Address")
  .requiredOption("-hook, --hook <string>", "Hook Program ID")
  .action(async (options) => {
    const sdk = await getSdk(program.opts());
    const tx = await sdk.setSss1TransferHook(
        new PublicKey(options.pda),
        new PublicKey(options.mint),
        new PublicKey(options.hook)
    );
    console.log(`SSS-1 Hook updated! Tx: ${tx}`);
  });
...
program
  .command("set-hook-sss2")
  .requiredOption("-p, --pda <string>", "Stablecoin PDA")
  .requiredOption("-m, --mint <string>", "Mint Address")
  .requiredOption("-hook, --hook <string>", "Hook Program ID")
  .action(async (options) => {
    const sdk = await getSdk(program.opts());
    const tx = await sdk.setSss2TransferHook(
        new PublicKey(options.pda),
        new PublicKey(options.mint),
        new PublicKey(options.hook)
    );
    console.log(`SSS-2 Hook updated! Tx: ${tx}`);
  });
...
program
  ...
  .requiredOption("-b, --blacklist <string>", "Blacklist PDA")
  .requiredOption("-a, --address <string>", "Address to remove from blacklist")
  .action(async (options) => {
    const sdk = await getSdk(program.opts());
    const tx = await sdk.removeFromBlacklist(new PublicKey(options.blacklist), new PublicKey(options.address));
    console.log(`Address removed from blacklist! Tx: ${tx}`);
  });

program
  .command("deploy")
  .description("Deploy all SSS programs using Anchor")
  .action(async () => {
    const { execSync } = require("child_process");
    console.log("Building and deploying all programs...");
    try {
        execSync("anchor build && anchor deploy", { stdio: "inherit", cwd: path.join(__dirname, "..") });
